...
def log_aggregation():
    ...
    try:
        ...
        green_trips_table = create_green_trips_source_kafka(t_env)
        ...
        FROM (
            SELECT
                PULocationID,
                DOLocationID,
                COUNT(*) OVER (PARTITION BY PULocationID, DOLocationID ORDER BY lpep_dropoff_datetime RANGE BETWEEN INTERVAL '5' MINUTE PRECEDING AND CURRENT ROW) AS streak_length
            FROM {green_trips_table}
        )
